...
**File:** [`Physical Robot Image`](./ARS_Project/Robot.jpeg)


## Documentation & Code

### 1. Hardware & Sensor Setup
**File:** [`RealSense-D455_YDLidar-X2_Setup_Guide.md`](./ARS_Project/RealSense-D455_YDLidar-X2_Setup_Guide.md)
...
**File:** [`3D-Mapping-SLAM-Layer_RTABMap-FastMapping_Setup_Guide.md`](./ARS_Project/3D-Mapping-SLAM-Layer_RTABMap-FastMapping_Setup_Guide.md)
...
> 📄 **For complete documentation, see [`HOW_TO_RUN.txt`](./ARS_Project/HOW_TO_RUN.txt)** — this file contains the full operational guide including SSH access, headless testing, and troubleshooting.
...
**Files:**
* [`Pinout_Reference.md`](./ARS_Project/Pinout_Reference.md) – Documents the Rev.F pin map for the Mecanum Drive and Encoders wired to an Arduino Mega 2560. Details the hardware connections for the Cytron MDDRC10 drivers and JGB37-520 motors.
* [`encoder_calibration.ino`](./ARS_Project/encoder_calibration.ino) – An Arduino utility to measure `TICKS_PER_REV` and derive `TICKS_PER_METER` for the four mecanum wheels It safely holds the motor drivers at neutral while wheels are turned manually for calibration.
* [`teleop_keyboard.py`](./ARS_Project/teleop_keyboard.py) – A hold-to-move keyboard teleoperation script utilizing `pynput`. It repeats the motion command at 20 Hz to feed the firmware's 200 ms watchdog timeout, immediately halting the robot when the key is released.
* [`teleop_ssh.py`](./ARS_Project/teleop_ssh.py) – An SSH-friendly keyboard teleoperation script that reads the terminal directly in raw mode, making it work without a graphical display. It uses latched control where tapping a direction keeps the robot moving and tapping space or 's' stops it.

### 5. Robot 3D Model

The robot's 3D printable components are provided as STL files:

- [`Base.stl`](./ARS_Project/Base.stl) – Main chassis of the robot.
- [`Roof.stl`](./ARS_Project/Roof.stl) – Top layer base for lidar.
- [`Piece3.2.STL`](./ARS_Project/Piece3.2.STL) – Additional mounting or structural component to connect the Lidar. 
